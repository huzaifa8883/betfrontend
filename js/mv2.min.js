...
// });
Vue.component('runner-aggregate', {
    props: ['runner', 'Catalog'],

    data() {
        return {
            displayPrices: {
                price1: parseFloat(this.runner.price1),
                price2: parseFloat(this.runner.price2),
                price3: parseFloat(this.runner.price3),
                lay1: parseFloat(this.runner.lay1),
                lay2: parseFloat(this.runner.lay2),
                lay3: parseFloat(this.runner.lay3)
            },
            timers: {} // store timers for each price/layer
        };
    },

    watch: {
        'runner.price1'(newVal) { this.smoothUpdate('price1', newVal); },
        'runner.price2'(newVal) { this.smoothUpdate('price2', newVal); },
        'runner.price3'(newVal) { this.smoothUpdate('price3', newVal); },
        'runner.lay1'(newVal) { this.smoothUpdate('lay1', newVal); },
        'runner.lay2'(newVal) { this.smoothUpdate('lay2', newVal); },
        'runner.lay3'(newVal) { this.smoothUpdate('lay3', newVal); },
    },

    methods: {
        smoothUpdate(key, newValue) {
            const target = parseFloat(newValue);
            if (isNaN(target)) return;

            // Stop previous animation if running
            if (this.timers[key]) {
                cancelAnimationFrame(this.timers[key]);
                this.timers[key] = null;
            }

            const step = () => {
                const current = this.displayPrices[key];
                const diff = target - current;

                // If difference is small, snap to target
                if (Math.abs(diff) < 0.01) {
                    this.displayPrices[key] = target;
                    return;
                }

                // Smooth blend factor (0.2 = 20%)
                this.displayPrices[key] = current + diff * 0.2;

                // Continue animation
                this.timers[key] = requestAnimationFrame(step);
            };

            this.timers[key] = requestAnimationFrame(step);
        },

        ShowBetSlip(runner, type, marketId, level) {
            this.$emit('betslip', { runner, type, marketId, level });
        }
    },
