...
data() {
    return {
        lastStatus: null, // Vue reactive state
        Catalog: {},      // agar pehle se nahi hai
        score: null,
        scores: [],
        scoreLines: [],
        fancySize: null,
...
methods: {
    updateMarketData(MarketData, topMarketId) {
        const Book = _.find(MarketData.marketBooks, book => book.id == topMarketId);
        const self = this;

        if (Book) {
            // Status handling
            const newStatus = Book.marketStatus;
            const oldStatus = this.lastStatus;

            if (!oldStatus) {
                this.Catalog.status = newStatus;
            } else if (oldStatus.toLowerCase() === 'suspended' && newStatus.toLowerCase() === 'open') {
                this.Catalog.status = oldStatus;
            } else {
                this.Catalog.status = newStatus;
            }

            if (['closed','suspended','open'].includes(newStatus.toLowerCase())) {
