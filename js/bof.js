...
if ($("#MarketData").length > 0) {
                ...
                if (Book !== undefined) {
                    ...
                    MarketVue.market.runners.forEach(function (runner) {
                        let soldier = Book.runners.find(function (item) {
                            return item.id.toString() === runner.selectionId.toString();
                        });
    ...
    $.getJSON(qs, (data) => {
        ...
        if (data && data.length > 0) {
            this.RenderSubMarket(data);
        } else {
            console.warn("No submarket data found for MarketId:", MarketId);
            $('#subMarketContainer').html("<p>No data available</p>");
        }
    });
},
...
function GetSports(id, type) {
    LastNavId = id;
    var qs = svc_prefix + 'markets/Navigation?id=' + id + '&type=' + type;
